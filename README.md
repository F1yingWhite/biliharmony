...
| 动态 | 关注动态 feed：顶部**关注 UP 主横滑栏**（头像直达用户空间 + 直播中红角标）、表情图文混排、九宫格图片、转发展示、直播卡片直达直播间；**点赞/转发**、**全部分类筛选（全部/视频/番剧/专栏）**、**发布纯文字/图文动态（选图九宫格，GIF 保真）**、**动态举报**（卡片「⋯」原因选择）、**推荐源切换**（Web 个性化 / App 推流，会话链防重复） |
...
- [x] 发布纯文字/图文动态（`DynamicView` FAB + 输入面板 + 相册选图九宫格，`upload_bfs` 上传 + `create/dyn` scene=2，GIF 原样保帧、静态图限边重编码）
