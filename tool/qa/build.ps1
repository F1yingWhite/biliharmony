...
$env:HOME = Join-Path $root '.hvigor_work'
$env:HVIGOR_USER_HOME = $env:HOME
$env:PATH = "$env:NODE_HOME;$env:JAVA_HOME\bin;$env:PATH"

$workspaceHvigor = Join-Path $env:HOME 'project_caches\98e35b12da4d7bd1e6d3e622451bd0f1\workspace\node_modules\@ohos\hvigor\bin\hvigor.js'
...
$argsList = @($workspaceHvigor, 'assembleHap', '--mode', 'module', '-p', 'product=default', '--no-daemon', '--no-parallel')

# hvigor 通过 -p/--prop 传参；避免参数数组解析问题，这里直接拼命令行
$cmdLine = "`"${node}`" `"$workspaceHvigor`" assembleHap --mode module -p product=default --no-daemon --no-parallel"
...
$hap = Get-Item (Join-Path $root 'entry\build\default\outputs\default\entry-default-unsigned.hap')
